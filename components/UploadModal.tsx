...
const UploadModal = () => {
    ...
    const onChange = (open: boolean) => {
        if (!open) {
            reset(); //Form will reset when modal is closed
            uploadModal.onClose()
        }
    }
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (values) => {
        ...
        try {
            ...
            if (songError) {
                setIsLoading(false)
                return toast.error("Could not upload song, Try again!!")
            }
            //Upload image
            ...
            if (imageError) {
                setIsLoading(false)
                return toast.error("Could not upload image, Try again!!")
            }
            const {
                error: supabaseError
    ...
    return (
        <Modal title='Add a Song' description='Upload a MP3 file' isOpen={uploadModal.isOpen}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-y-4'>
                ...
                <Button disabled={isLoading} type='submit'>Upload Song</Button>
            </form>
        </Modal>
    )
}
